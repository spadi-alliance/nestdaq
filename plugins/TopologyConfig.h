...
private:
  void DeleteProperty(const std::string& key) { fPlugin.DeleteProperty(key); }
  std::shared_ptr<sw::redis::Redis> GetClient() const { return fPlugin.GetClient(); }
  std::mutex& GetMutex() { return fPlugin.GetMutex(); }
  std::map<std::string, std::string> GetPropertiesAsStringStartingWith(const std::string& q) const { return fPlugin.GetPropertiesAsStringStartingWith(q); }
  template <typename T> T GetProperty(const std::string& key) const { return fPlugin.GetProperty<T>(key); }
  void Initialize();
  void InitializeDefaultChannelProperties();
  bool IsCanceled() const { return fPlugin.IsCanceled(); }
  int PropertyExists(const std::string& key) { return fPlugin.PropertyExists(key); }
  ...
  const std::vector<std::string> ReadPeerAddress(const std::string &peerHealthKey, std::string_view peerIP, std::string_view peerChannel);
  void ResolveConnectAddress();
  void SetProperties(const fair::mq::Properties &props) { fPlugin.SetProperties(props); }
  void Unregister();
