...
public:
  ...
  WebGui() { InitializeFunctionList(); }
  WebGui(const WebGui &) = delete;
  WebGui& operator=(const WebGui &) = delete;
  ~WebGui() { Send(0, "Disconnected."); }

  // add function to the list for ProcessData
  void AddFunction(const std::string& command, ProcessDataFunc f) { fFuncList.emplace(command, f); }
  void AddFunction(const std::unordered_map<std::string, ProcessDataFunc>& table) { fFuncList.insert(table.cbegin(), table.cend()); }
  ...
  const std::string& GetChannelPrefix() const { return fChannelName; }
  const std::list<std::pair<unsigned int, std::string>>& GetWebSocketIdList() const  { return fWebSocketIdList; }
  const std::string& GetSeparator() const { return fSeparator; }
  std::shared_ptr<sw::redis::Redis> GetRedisClient() { return fClient; }
  ...
  void Send(unsigned int connid, const std::string& arg) { fSend(connid, arg); }
